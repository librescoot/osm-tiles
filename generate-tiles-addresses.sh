...
generate_tiles() {
    ...
    echo "Generating address tiles for $name..."
    docker run --rm \
        -v "$(pwd):/var/tm" \
        -v "/tmp:/tmp" \
        -w /var/tm \
...
echo "=== Generating address tiles for German states ==="
for entry in "${STATES[@]}"; do
    state="${entry%%|*}"
    url="${entry##*|}"
    pbf_file="/tmp/${state}.osm.pbf"
    output_file="$OUTPUT_DIR/tiles_addresses_${state}.mbtiles"
...
echo ""
echo "=== Generating address tiles for all of Germany ==="
germany_pbf="/tmp/germany-latest.osm.pbf"
germany_output="$OUTPUT_DIR/tiles_addresses_germany.mbtiles"
...
echo ""
echo "=== All address tiles generated successfully ==="
